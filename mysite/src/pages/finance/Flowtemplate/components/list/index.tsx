...
import { ArrowDownOutlined, ArrowUpOutlined, createFromIconfontCN, MinusOutlined, PlusOutlined } from '@ant-design/icons';
...
const List = React.forwardRef((props: ListProps, ref) => {
  ...
  const handleRemove = async (selectedRows: TableListItem[], deleteRow: any) => {
    ...
    try {
      ...
      if (selectedRows.length > 0) {
        deleteIds = selectedRows.map((row:any) => row.id.toString());
  ...
  const deleteItem = (item: any) => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        removeFlowtemplate({id:item.id}).then((res)=>{
          if (res.errorMessage) {
            Modal.error({
              title: '报错',
  ...
  const modifyApprovalStepItem = async (values:any)=>{
    ...
    // 如果values['role']不是数字
    if(isNaN(values['role'])&&items.type==0){
      // 遍历roleDict找到对应的key
      for(let key in roleDict){
        if(roleDict[key].text===values['role']){
          values['role']=parseInt(key);
          break;
        }
      }
    }
    ...
    items.notnull=values['notnull']?1:0;
  ...
  const moveApprovalStepItem = (item:any,direction:any)=>{
    console.log('移动：',item)
    const newData:any = currentRow;
    const index = currentRow?.templateData.approval.findIndex((i:any) => i.key == item.key)
    if (index === -1) {
      return ; // 元素不在数组中，直接返回原数组
    }

  // 向上移动
  if (direction === 'up') {
    if (index > 0) {
      [newData.templateData.approval[index], newData.templateData.approval[index - 1]] = [newData.templateData.approval[index - 1], newData.templateData.approval[index]]; // 交换位置
    }
  }
  // 向下移动
  else if (direction === 'down') {
    if (index < newData.templateData.approval.length - 1) {
      [newData.templateData.approval[index], newData.templateData.approval[index + 1]] = [newData.templateData.approval[index + 1], newData.templateData.approval[index]]; // 交换位置
    }
  }
    setAkey(akey+1)
  ...
  const modifyNotifyStepItem = async (values:any)=>{
    ...
    const index = currentStep?newData.findIndex((item:any) => currentStep.key === item.key):newData.length;
    const items = currentStep?newData[index]:{};
    items.type=parseInt(stepTypeVal)||0;
    items.role=parseInt(values['role'])||0;
    ...
    },
    {
      ...
      render: (_, entity:any) => 
        ...
        <ArrowUpOutlined onClick={()=>{
          moveApprovalStepItem(entity,'up')
        }} />
        <ArrowDownOutlined onClick={()=>{
          moveApprovalStepItem(entity,'down')
        }}/>
        </Space>,
  ...
  const notifyColumns = [
    ...
    },
    {
      ...
      dataIndex: 'option',
      valueType: 'option',
      render: (_, entity:any) => <Space size="middle">
        <a
          key="nedit"
          onClick={() => {
            setCurrentStep(entity);
  ...
  return (
    <>
      ...
      <DrawerForm
        ...
        onFinish={async (values) => {
          const result:any = await handleAddAndUpdate(currentRow, values);
          if (result) {
            if (result.errorCode) {
              return false;
            }
        ...
        initialValues={{
          ...
          notifyAttr:currentRow?.notifyAttr||2,
        }}
        layout="vertical"
        grid={true}
      >
    ...
    <Tabs defaultActiveKey="1">
          <TabPane tab="默认审批人" key="1" forceRender>
            ...
            <Table key={akey} dataSource={approvalData} columns={approvalColumns} pagination={false} />
          </TabPane>
      ...
      <ModalForm
        ...
        initialValues={{
          ...
          attr:currentStep &&  currentStep.attr && currentStep.attr.toString(),
          skip:currentStep && currentStep.skip==1?true:false,
          span:currentStep && currentStep.span==1?true:false,
          notnull:currentStep && currentStep.notnull==1?true:false,
        }}
        layout="vertical"
        grid={true}
      >
      ...
      <Tabs activeKey={stepTypeVal} onChange={stepTypeChange}>
        ...
        <ProForm.Group>
          <ProFormSwitch name="skip" label="是否跨部门" />
        </ProForm.Group>
        <ProForm.Group>
          <ProFormSwitch name="notnull" label="解析时不能为空" />
        </ProForm.Group>
      </ModalForm>
      <ModalForm
        title="编辑抄送信息"
