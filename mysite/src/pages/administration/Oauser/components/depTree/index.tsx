...
const DepTree = React.forwardRef((props: DepartmentTreeProps, ref) => {
  const myProps = props;
  ...
  // const rootId = props.rootId ? props.rootId : 0;
      ...
      if (parentId == 0 || (props.rootId && !treeNode)) {
        ...
        if (props.onSelect) {
          setTimeout(() => {
            ...
            myProps.onSelect({id:rootKey,title:result.data[0].title});
  ...
  const onSelect: TreeProps['onSelect'] = (keys: any,info:any) => {
    if (keys.length > 0) {
      ...
      if (props.onSelect) {
        ...
        props.onSelect({id:keys[0],title:info.node.title});
