  ...
  <div class="page">
    ...
    <Tab   title="已审批">
      <div style="display: flex;flex-direction: row;">
        ...
        <span style="background-color: #1989fa;width: calc(var(--van-cell-font-size)*4);color: white;text-align: center;padding: 5px;margin: 5px;
          font-size: var(--van-cell-font-size);border-radius: 10%;" @click="onSearch(1)">搜索</span>
      </div>
      <List
        :loading="loading[1]"
        :finished="finished[1]"
...
import listitem from '@/views/useseal/components/listitem.vue';
...
const cacheStore = useUserStore()
  export default {
    ...
    mounted() {
      ...
      var tab = cacheStore.userInfo?.usesealIndexTab
    ...
    methods:{
      ...
      viewHistory(e:any){
        ...
        this.$router.push({name:'useseal_view',query:q})
      },
      view(e:any){
 
        this.$router.push({name:'useseal_view',query:{thirdNo:e.thirdNo}})
      },
