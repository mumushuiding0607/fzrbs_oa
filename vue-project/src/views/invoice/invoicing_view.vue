...
<template>
  <div class="box">
    <Viewflow :invoicingid="par.invoicingid" :info="data" :thirdNo="par.thirdNo||thirdNo" />
  </div>
</template>
<script  lang="ts">

import { Tabs,Tab,NavBar } from 'vant';
import Viewflow from './components/invoicing_viewflow.vue';
import { getbyid } from './invoice';
  export default {
    components: {
      Viewflow,NavBar,
      Tabs,Tab,Image
    ...
    mounted() {
      getbyid({id:this.par.invoicingid}).then((res:any)=>{
        if (res) {
    ...
    methods:{
      resize(){
        ...
        if (elementWidth>500){
          this.marginLeft = (elementWidth-500)/2-7
          this.width = '496px'
        }else{
          this.marginLeft = 0
          this.width = '100%'
