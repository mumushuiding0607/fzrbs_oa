...
use app\modules\api\commons\Tools as ApiTools;

/**
 * 采编管理“三重一大”内容相关接口类
 */
class InstructionDisposalController extends ApiBase
{
    public $enableCsrfValidation = false;
    protected $_orderBy = 'id desc';
    protected $_agentId = 1000082;
    ...
    public function actionList()
    {
        ...
        foreach ($res as $k => $row) {
            ...
            $item = [
                'title' => $this->_department_name[$flowdata['typeid']] . '-采编管理“三重一大”内容记录表',
    ...
    public function actionSave()
    {
        if (isset($this->_request['wxuserid'])) {
            ...
            if (!$action) {
                ...
                $textcard = $this->getTextCard('采编管理“三重一大”内容记录表审核申请', $request, $thirdNo);
                try {
        ...
        if ($thirdNo) {
            ...
            if ($ret) {
                ...
                if ($ret['isfinish']) {
                    ...
                    $textcard = $this->getTextCard('您提交的采编管理“三重一大”内容记录表申请已通过', $info, $thirdNo);
                    try {
                        $sendResult = Tools::sendWxMessage($this->_agentId, $touserstr, $textcard, '', 'textcard');
                    } catch (\Exception $e) {
                    }
                    ...
                    $textcard = $this->getTextCard($data['userName'] . '提交的采编管理“三重一大”内容记录表申请需要您审核', $info, $thirdNo);
                    try {
                        $sendResult = Tools::sendWxMessage($this->_agentId, $touserstr, $textcard, '', 'textcard');
                    } catch (\Exception $e) {
                    }
    ...
    public function actionReject()
    {
        ...
        if ($thirdNo) {
            ...
            if ($ret) {
                //发送消息
                $touserstr = $data['userId'];
                $textcard = $this->getTextCard($this->_userInfo->name . '驳回了您提交的采编管理“三重一大”内容记录表申请', $info, $thirdNo);
                try {
                    $sendResult = Tools::sendWxMessage($this->_agentId, $touserstr, $textcard, '', 'textcard');
                } catch (\Exception $e) {
                }
                ...
                if ($toUser) {
                    ...
                    $textcard = $this->getTextCard($this->_userInfo->name . '驳回了' . $data['userName'] . '提交的采编管理“三重一大”内容记录表申请', $info, $thirdNo);
                    try {
                        $sendResult = Tools::sendWxMessage($this->_agentId, $touserstr, $textcard, '', 'textcard');
                    } catch (\Exception $e) {
                    }
    ...
    public function actionCancel()
    {
        ...
        if ($thirdNo) {
            ...
            if ($ret) {
                ...
                if ($toUser) {
                    $touserstr = implode('|', $toUser);
                    $textcard = $this->getTextCard('撤消采编管理“三重一大”内容记录表申请', $info, $thirdNo);
                    try {
                        $sendResult = Tools::sendWxMessage($this->_agentId, $touserstr, $textcard, '', 'textcard');
                    } catch (\Exception $e) {
                    }
                ...
                $info['method'] = 'edit';
                $touserstr = $data['userId'];
                $textcard = $this->getTextCard('您撤消了采编管理“三重一大”内容记录表申请，点击可重新申请', $info, $thirdNo);
                try {
                    $sendResult = Tools::sendWxMessage($this->_agentId, $touserstr, $textcard, '', 'textcard');
                } catch (\Exception $e) {
                }
     ...
     * 催办审核内容
     */
    public function actionUploadFileData()
    {
        $filename = $this->_request['filename'];
        if ($filename && $this->_UserId) {
            $localFile = $this->_fileSavePath . $filename;
            if (file_exists($localFile)) {
                $data = ApiTools::getExcelData($localFile);
                if ($data) {
                    $successNum = 0;
                    $order = 1;
                    $importdata = [];
                    ...
                    for ($i = $startRowNum; $i <= $count; $i++) {
                        ...
                        $czqk = trim($data[$i][2]);
                        if ($bt && $zlly && $czqk) {
                            $importdata[] = ['order' => $order, 'title' => $bt, 'source' => $zlly, 'handle' => $czqk];
                            $order++;
                        }
                    }
    ...
    private function getTextCard($title, $data, $thirdNo)
    {
        ...
        return array(
            ...
            'url' => 'https://api.fznews.com.cn/weixin/work-web-oauth/index?backurl=https://fzrb.fznews.com.cn/v2/szyd/edit?method=' . $method . '&thirdNo=' . $thirdNo,
            'btntxt' => '详情'
        );
    }
