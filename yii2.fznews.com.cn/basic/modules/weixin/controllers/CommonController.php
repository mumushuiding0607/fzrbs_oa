...
class CommonController extends ApiBase
{
    ...
    {
        $userId = $this->_request['userid'] ? $this->_request['userid'] : 0;
        $this->_result['data'] = $this->_getUserInfo($userId);
        Tools::responseJson($this->_result);
    }

    public function actionPreviewFile()
    {
        $fileurl = $this->_request['fileurl'];
        $protect = isset($this->_request['protect']) ? true : false;
        $stream = isset($this->_request['stream']) ? true : false;
        
        $data = Tools::getFileData($fileurl, $protect, $stream);
            ...
            if (intval($this->_request['firstRequest']) === 1 && $parentId > 0) {
                ...
                foreach ($res as $row) {
                    ...
                    if (intval($this->_request['user']) === 1) {
                        $users = WeixinOAUserInfo::find()->where(['and', ['=', 'departmentid', $row->id], ['=', 'st', 1]])->orderBy('displayorder desc')->all();
                        if ($users) {
                            $node['isLeaf'] = false;
                ...
                if (intval($this->_request['user']) === 1) {
                    $users = WeixinOAUserInfo::find()->where(['and', ['=', 'departmentid', $parentId], ['=', 'st', 1]])->orderBy('displayorder desc')->all();
                    if ($users) {
                        foreach ($users as $user) {
                            $data[] = ['title' => $user['name'], 'key' => $user['userid'], 'value' => $user['userid'], 'isLeaf' => true, 'avatar' => $user['avatar']];
                        }
                    }
                }
            }
            if (isset($this->_request['nav'])) {
                $row = WxDepartment::find()->where(['=', 'id', $parentId])->orderBy('order desc')->one();
                if ($row) {
                    $res = WxDepartment::find()->select('id,name')->where(['or', ['=', 'id', $parentId], ['in', 'id', explode(',', $row['parentids'])]])->orderBy('id asc')->all();
                    $this->_result['nav_data'] = $res;
                }
            }
            $this->_result['data'] = $data;
        } else {
    ...
    public function actionSearchUser()
    {
        ...
        $users = WeixinOAUserInfo::find()->select('id,userid,name,avatar,departmentname')->where($where)->orderBy('displayorder desc')->all();
    ...
    protected function _getLocalDepartmentChildren($parentId)
    {
        ...
        foreach ($res as $row) {
            ...
            if (intval($this->_request['user']) === 1) {
                ...
                if ($users) {
                    foreach ($users as $user) {
                        $children[] = ['title' => $user['name'], 'key' => $user['userid'], 'value' => $user['userid'], 'isLeaf' => true, 'avatar' => $user['avatar']];
                    }
                }
            }
            if ($children) {
